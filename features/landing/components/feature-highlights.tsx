...
const FEATURES = [
  { kind: "weather", eyebrow: "Weather Intelligence", title: "Know what is coming to every field.", description: "View farm-local forecasts with clearly labeled sources and update times." },
  { kind: "irrigation", eyebrow: "Irrigation planning", title: "Plan when your fields need water.", description: "Save irrigation dates and durations for each farm. Automatic watering and device control are not yet available." },
  ...
  { kind: "ai", eyebrow: "AI Recommendations", title: "Move from raw signals to the next best action.", description: "AI recommendations are planned. The assistant does not generate advice yet." },
] as const;
  ...
  return (
    <Section className="py-24 sm:py-32">
      <Reveal className="max-w-3xl" >
        ...
        <p className="mt-5 text-base leading-7 text-muted-foreground sm:text-lg"><T text="Farm management, weather and irrigation planning are available today. AI and connected devices are planned." /></p>
      </Reveal>
          ...
          <Reveal key={feature.kind}>
            <article className="grid items-center gap-9 lg:grid-cols-2 lg:gap-20">
              <div className={index % 2 ? "lg:order-2" : ""}>
                ...
                <div className="mt-7 flex items-center gap-3 text-sm font-semibold text-primary"><Sparkles className="size-4" /><T text={feature.kind === "ai" ? "Coming soon" : "Available now"} /></div>
              </div>
              <div className={index % 2 ? "lg:order-1" : ""}><p className="mb-3 text-xs font-medium text-muted-foreground"><T text="Illustrative preview · Sample data" /></p><ProductVisual kind={feature.kind} /></div>
            </article>
          </Reveal>
        ))}
      </div>
...
function ProductVisual({ kind }: { kind: typeof FEATURES[number]["kind"] }) {
  const { format } = useSettings();
  const t = useTranslation();
  const shell = `relative overflow-hidden rounded-[2rem] border border-primary/10 bg-linear-to-br from-card via-card to-primary/8 p-5 shadow-[0_24px_70px_rgba(11,70,47,0.12)] sm:p-7`;
