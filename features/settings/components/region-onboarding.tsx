...
import type { Country, Language } from "../types/preferences";
...
function RegionChoice() {
  ...
  const [country, setCountry] = useState<Country>(
    () => navigator.languages.map(countryFromLocale).find(Boolean) ?? settings.country,
  );
  const [language, setLanguage] = useState<Language>(() =>
    settings.language === "fa" ? "fa" : "en",
  );
  const [regionSource, setRegionSource] = useState<"manual" | "detected">("manual");
  ...
  const detect = async () => {
    ...
    try {
      ...
      if (revision === choiceRevision.current)
        setMessage(t("Location unavailable. You can choose your region below."));
    } finally {
      setBusy(false);
    }
  };
      ...
      <Dialog.Portal>
        ...
        <Dialog.Popup className="fixed start-1/2 top-1/2 z-[100] max-h-[90dvh] w-[calc(100%_-_2rem)] max-w-lg -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-3xl border bg-card p-6 shadow-2xl outline-none rtl:translate-x-1/2 sm:p-8">
          ...
          <Dialog.Title className="text-2xl font-bold">
            {t("Make AgroMind yours")}
          </Dialog.Title>
          <Dialog.Description className="my-3 text-sm leading-6 text-muted-foreground">
            {t(
              "Choose your region for local examples and default units. Language and units can be changed separately in Settings.",
            )}
          </Dialog.Description>
          <div className="space-y-4">
            <PreferenceSelect
              ...
              options={COUNTRY_CODES.map((value) => ({
                value,
                label: countryDisplayName(value, REGION_PROFILES[country].locale),
          ...
          <button
            ...
            disabled={busy}
            onClick={detect}
            className="my-4 flex items-center gap-2 text-sm font-medium text-primary disabled:opacity-50"
          >
            <LocateFixed size={17} />
            {t(busy ? "Detecting region…" : "Suggest from my location")}
          </button>
          <p role="status" className="text-sm text-muted-foreground">
            {message}
          </p>
          <div
            className="mt-4 space-y-2 rounded-2xl bg-primary/5 p-4 text-sm"
            dir={preview.direction}
          >
            <p className="font-semibold">
              {preview.language === "fa" ? t("Persian · right to left") : t("English")}
            </p>
            <p dir="ltr">
              {format.measure(27, "temperature")} · {format.symbol("area")} ·{" "}
              {format.symbol("wind")} · {format.symbol("precipitation")}
            </p>
          ...
          <button
            ...
            className="mt-6 flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-primary px-5 py-3 font-semibold text-primary-foreground"
          >
            <Sprout size={18} />
            {t("Continue")}
          </button>
...
export function RegionOnboarding() {
  ...
  return path === "/" && isHydrated && !preferences.regionConfirmed ? (
    <RegionChoice />
  ) : null;
}
