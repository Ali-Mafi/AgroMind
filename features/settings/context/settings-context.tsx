...
const setUnit: SettingsContextValue["setUnit"] = (kind, unit) =>
  updatePreferences((previous) => ({
    ...previous,
    units: { ...previous.units, [kind]: unit },
  }));
...
export function SettingsProvider({
  ...
  // Server preferences prevent hydration mismatches, then the client-side store
  // becomes authoritative so changes made in Settings remain visible immediately.
  const preferences = isHydrated
  ...
  const value = useMemo(() => {
    ...
    return {
      ...
      format: createFormatters(resolved),
      update: updatePreferences,
      setUnit,
      areaUnit: resolved.units.area,
      setAreaUnit: (unit: AreaUnit) => setUnit("area", unit),
    };
