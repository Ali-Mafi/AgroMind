...
function harness({
  error = null,
  user = { id: "user-a", email_confirmed_at: "2026-09-13" },
  completed = false,
} = {}) {
  const calls = [];
  const record =
...
test("signup validates all fields and never returns password values", async () => {
  const h = harness();
  for (const input of [
    { ...signup, email: "wrong" },
    { ...signup, fullName: " " },
...
test("signup delegates credentials to Supabase and returns the same pending destination for duplicates", async () => {
  for (const error of [
    null,
    { code: "user_already_exists", status: 422, message: "private detail" },
  ]) {
    ...
    const sent = h.calls[0].args[0];
    assert.equal(
      sent.options.emailRedirectTo,
      "https://agromind.ir/auth/callback",
    );
...
test("signup fails closed if email confirmation was accidentally disabled", async () => {
  const h = harness();
  h.client.auth.signUp = async () => ({ data: { session: {} }, error: null });
  const result = await h.actions.signUpAction({}, form(signup));
  assert.match(result.error, /unavailable/);
  assert.equal(h.calls[0].name, "signOut");
});
test("login resumes onboarding and sanitizes the requested destination", async () => {
  for (const [completed, next, expected] of [
    ...
    await assert.rejects(
      h.actions.signInAction({}, form({ ...signup, next })),
  ...
  assert.match(
    (await h.actions.signInAction({}, form(signup))).error,
    /Email or password is incorrect/,
  );
});
test("forgot and resend suppress account enumeration and cooldown details", async () => {
  const results = [];
  for (const error of [
    null,
    ...
    assert.deepEqual(
      await h.actions.resendVerificationAction(
        {},
        form({ email: signup.email }),
      ),
      results[0],
...
test("verification uses Supabase OTP verification and handles expired and invalid links", async () => {
  ...
  assert.match(
    (await expired.actions.verifyEmailAction({}, form({ token_hash: token })))
      .error,
...
for (const language of ["en", "fa"])
  test(
    ...
    () => {
      ...
      const { AuthForm } = load(
        "features/authentication/components/auth-form.tsx",
      );
      ...
      for (const mode of [
        ...
        if (mode === "sign-up") assert.match(html, /name="confirmPassword"/);
      }
    },
  );
