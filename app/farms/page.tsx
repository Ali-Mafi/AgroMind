...
import { MapPin, Plus, Sprout, TreePine } from "lucide-react";
  ...
  return <main className="app-page"><PageHeader title={t("My Farms")} description={t("A place for everything you grow.")} action={farms.length ? add : undefined} />
    {farms.length ? <section aria-label={t("My farms and gardens")} className="grid gap-5 md:grid-cols-2 2xl:grid-cols-3">{farms.map(farm => {
      ...
      return <article key={farm.id} className="app-card overflow-hidden"><Link href={`/farms/${farm.id}`} className="app-card-link block h-full p-5 sm:p-6">
        <div className="mb-7 flex items-center justify-between gap-3"><span className="flex size-11 items-center justify-center rounded-2xl bg-primary/8 text-primary">{garden ? <TreePine size={23} /> : <Sprout size={23} />}</span><div className="flex flex-wrap justify-end gap-2"><span className="rounded-full bg-muted px-3 py-1 text-xs text-muted-foreground">{t(garden ? "Garden" : "Farm")}</span>{!farm.access.owned && <span className="rounded-full bg-primary/8 px-3 py-1 text-xs font-medium text-primary">{t(farm.access.role === "manager" ? "Manager" : farm.access.role === "worker" ? "Worker" : "Viewer")}</span>}</div></div>
